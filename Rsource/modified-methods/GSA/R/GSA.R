...
censoring.status=NULL,random.seed=NULL,  knn.neighbors=10,
  s0=NULL, s0.perc=NULL,minsize=15,maxsize=500, restand=TRUE,restand.basis=c("catalog","data"), nperms=200, xl.mode=c("regular","firsttime","next20","lasttime"), xl.time=NULL, xl.prevfit=NULL, ncores = 1){
#
...
if(xl.mode=="regular"){
  ...
  mod.tmp <- mclapply(X = first:last, mc.cores = ncores, FUN = function(i){
    ...
    if(resp.type=="Two class paired"){
      ...
      oo=sample(1:length(y))
    }
    
    GSA.func(x[,oo],y,genesets=genesets, genenames=genenames, 
  ...
  for (i in first:last){
    r.star[,i]=mod.tmp[[i]]$score
    if (restand==TRUE) {  
      stand.info.star[, i] = unlist(mod.tmp[[i]]$stand.info)
