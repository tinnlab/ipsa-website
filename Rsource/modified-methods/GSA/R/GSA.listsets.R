...
geneset.names = substring(geneset.names, 1, maxchar)
    ...
    for (i in oo) {
    if(negflag){ fdr.lo[i] = m * pvalues.lo[i]/sum(pvalues.lo[!is.na(pvalues.lo)] <=
            pvalues.lo[i]) }
        fdr.hi[i] = m * pvalues.hi[i]/sum(pvalues.hi[!is.na(pvalues.hi)] <= 
            pvalues.hi[i])
    }
 ...
 fdr.hi=pmax(fdr.hi,0)
    ...
    if(negflag){
        ...
        res1 = NULL
        for (i in oo1) {
                res1 = rbind(res1, c(i, geneset.names[i], GSA.obj$GSA.scores[i], pvalues.lo[i], fdr.lo[i]))
        }
    ...
    res2 = NULL
    for (i in oo2) {
            res2 = rbind(res2, c(i, geneset.names[i], GSA.obj$GSA.scores[i], pvalues.hi[i], fdr.hi[i]))
    }
