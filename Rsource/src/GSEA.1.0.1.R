...
GSEA.GeneRanking <- function(A, class.labels, gene.labels, nperm, permutation.type = 0, sigma.correction = "GeneCluster", fraction=1.0, replace=F, reverse.sign= F) {
    ...
    if (sigma.correction == "GeneCluster") {  # small sigma "fix" as used in GeneCluster
        ...
        #Remove ifelse
        idx <- !(S2 > 0.2*abs(M2))
        S2[idx] <- 0.2*abs(M2)[idx]
        S2[S2 == 0] <- 0.2
        idx <- !(S1 > 0.2*abs(M1))
        S1[idx] <- 0.2*abs(M1)[idx]
        S1[S1 == 0] <- 0.2
    }
...
GSEA.EnrichmentScore2 <- function(gene.list, gene.set, weighted.score.type = 1, correl.vector = NULL, gene.list.order) {
    #
    ...
    loc.vector <- gene.list.order #order(gene.list)
    ...
    tag.loc.vector <- tag.loc.vector[order(tag.loc.vector)] #sort(tag.loc.vector, decreasing = F)
...
GSEA <- function(
    ...
    for (k in 1:Ng) {
        ...
        if(ES.value > 0)
        {
            ...
            count.col[count.col.norm > 0] <- (colSums(phi.norm >= ES.value)/count.col.norm)[count.col.norm > 0]
            obs.count.col[obs.count.col.norm > 0] <- (colSums(obs.phi.norm >= ES.value)/obs.count.col.norm)[obs.count.col.norm > 0]
    ...
    for (k in 1:Ng) {
        ...
        if (NES[k] >= 0) {
            ...
            NULL.pass[NULL.pos > 0] <- (colSums(phi.norm >= NES[k])/NULL.pos)[NULL.pos > 0]
            OBS.pos <- OBS.pos.pos
            OBS.pass <- rep(0, nperm)
            OBS.pass[OBS.pos > 0] <- (colSums(obs.phi.norm >= NES[k])/OBS.pos)[OBS.pos > 0] 
