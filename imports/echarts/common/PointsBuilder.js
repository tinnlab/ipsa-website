...
import spriteUtil from 'echarts-gl/src/util/sprite'
import PointsMesh from 'echarts-gl/src/chart/common/PointsMesh'
import LabelsBuilder from 'echarts-gl/src/component/common/LabelsBuilder'
import Matrix4 from 'claygl/src/math/Matrix4'
import retrieve from 'echarts-gl/src/util/retrieve'
import {
  getItemVisualColor,
  getItemVisualOpacity,
} from 'echarts-gl/src/util/visual'
import { getVisualColor, getVisualOpacity } from 'echarts-gl/src/util/visual'
var SDF_RANGE = 20
var Z_2D = -10
...
PointsBuilder.prototype = {
  ...
  update: function (seriesModel, ecModel, api, start, end) {
    ...
    if (symbolInfo.maxSize > 2) {
      pointSizeScale = this._updateSymbolSprite(
        seriesModel,
        itemStyle,
        symbolInfo,
        dpr,
    ...
    //   material.set({
    //     color: rgbaArr,
    //     u_Size: symbolInfo.maxSize * this._sizeScale * 2,
    //   })
    // } else {
    material.set({
      color: [1, 1, 1, 1],
    ...
    for (var i = 0; i < f; i++) {
      ...
      // symbolSize =
      //   symbolSize instanceof Array
      //     ? Math.max(symbolSize[0], symbolSize[1])
      //     : symbolSize // NaN pointSize may have strange result.
      //
      // if (isNaN(symbolSize)) {
      //   symbolSize = 0
  ...
  _updateSymbolSprite: function (seriesModel, itemStyle, symbolInfo, dpr) {
    ...
    if (
      ...
      spriteUtil.createSDFFromCanvas(
        this._spriteImageCanvas,
        Math.min(this._spriteImageCanvas.width, 32),
        SDF_RANGE,
  ...
  highlight: function (data, dataIndex) {
    ...
    this._mesh.geometry.attributes.color.set(
      dataIndex - this._startDataIndex,
      colorArr,
    )
  ...
  setSizeScale: function (sizeScale) {
    if (sizeScale !== this._sizeScale) {
      if (this._mesh) {
        ...
        this._mesh.material.set(
          'u_Size',
          (originalSize / this._sizeScale) * sizeScale,
        )
        ...
        if (attributes.size.value) {
          for (var i = 0; i < attributes.size.value.length; i++) {
            attributes.size.value[i] =
              (attributes.size.value[i] / this._sizeScale) * sizeScale
          }
        }
      }
  ...
  _setPositionTextureToMesh: function (mesh, texture) {
    ...
    mesh.material[texture ? 'enableTexture' : 'disableTexture'](
      'positionTexture',
    )
  },
  _getSymbolInfo: function (seriesModel, start, end) {
    ...
    for (var idx = start; idx < end; idx++) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        if (
          symbolAspect != null &&
          Math.abs(currentSymbolAspect - symbolAspect) > 0.05
        ) {
